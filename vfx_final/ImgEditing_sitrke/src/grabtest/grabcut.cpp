...
GrabCut::GrabCut( GmmImage<GmmColor>* image, int nc_fore, int nc_back, double smooth) 
{
	m_image = image;
	...
	m_trimap = new GmmImage<TrimapValue>( m_w, m_h );
	m_trimap->fill(TrimapUnknown);

	m_GMMcomponent = new GmmImage<unsigned int>( m_w, m_h );

	m_hardSegmentation = new GmmImage<SegmentationValue>( m_w, m_h );
	m_hardSegmentation->fill(SegmentationBackground);

	m_softSegmentation = 0;		// Not yet implemented

	m_TLinksImage = new GmmImage<GmmColor>(m_w, m_h);
	m_TLinksImage->fill(GmmColor(0,0,0));
	m_NLinksImage = new GmmImage<Real>(m_w, m_h);
	m_NLinksImage->fill(0);
	m_GMMImage = new GmmImage<GmmColor>(m_w, m_h);
	m_GMMImage->fill(GmmColor(0,0,0));
	m_AlphaImage = new GmmImage<Real>(m_w, m_h);
	m_AlphaImage->fill(0);

	m_foregroundGMM = new GMM(nc_fore);
	m_backgroundGMM = new GMM(nc_back);
	...
	m_NLinks = new GmmImage<NLinks>( m_w, m_h );
	computeNLinks();

	m_graph = 0;
	m_nodes = new GmmImage<GraphType::node_id>( m_w, m_h );
}
...
void GrabCut::initGraph()
{
	...
	if (m_graph)
		delete m_graph;
	m_graph = new GraphType(m_w*m_h, m_w*m_h);
	...
	for (unsigned int y = 0; y < m_h; ++y)
	{
		for (unsigned int x = 0; x < m_w; ++x)
		{
			...
			if ((*m_hardSegmentation)(x,y) == SegmentationForeground)
				(*m_GMMImage)(x,y) = GmmColor((Real)((*m_GMMcomponent)(x,y)+1)/m_foregroundGMM->K(),0,0);
			else
				(*m_GMMImage)(x,y) = GmmColor(0,(Real)((*m_GMMcomponent)(x,y)+1)/m_backgroundGMM->K(),0);
